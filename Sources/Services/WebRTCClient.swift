    ...
    func startCaptureLocalVideo(renderer: RTCVideoRenderer) {
        ...
        guard
            ...
            let backCamera = (RTCCameraVideoCapturer.captureDevices().first { $0.position == .back }),
                ...
                // Filter formats to find 640x480 resolution
            let format = (RTCCameraVideoCapturer.supportedFormats(for: backCamera).first { format in
...
//    private func processReceivedPoints(_ points: [Point]) {
//        // Example: Log or send the points to a UI renderer
//        for point in points {
//            debugPrint("Point received: x=\(point.x), y=\(point.y)")
//        }
//        
//        // Notify a delegate or update UI
//        DispatchQueue.main.async {
//            // Example: Call a UI method to draw the points
//            self.delegate?.webRTCClient(self, didReceivePoints: points)
//        }
//    }
    
    func stopCaptureLocalVideo() {
        // Disable the local video track to stop sending frames
        self.localVideoTrack?.isEnabled = false

        // Stop the video capturer (camera)
        if let capturer = self.videoCapturer as? RTCCameraVideoCapturer {
            capturer.stopCapture()
        }
    }
    
    func resumeCaptureLocalVideo() {
        // Enable the local video track to send frames
