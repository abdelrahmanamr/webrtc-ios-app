...
class MainViewController: UIViewController {
    ...
    @IBAction private func videoDidTap(_ sender: UIButton) {
        // Instantiate a new VideoViewController each time the button is tapped
        self.webRTCClient.resumeCaptureLocalVideo()
        let newVideoViewController = VideoViewController(webRTCClient: self.webRTCClient)
           
        // Present the new VideoViewController
        self.present(newVideoViewController, animated: true, completion: nil)
...
extension MainViewController: SignalClientDelegate {
    ...
    func signalClient(_ signalClient: SignalingClient, didReceiveRawPoints rawPoints: RawPoints) {
        self.webRTCClient.receivedPoints = rawPoints.points
        print("Received raw points: \(rawPoints.points)")
    }
}
...
extension MainViewController: WebRTCClientDelegate {
    ...
    func webRTCClient(_ client: WebRTCClient, didReceivePoints points: [Point]) {
        for point in points {
            print("Point received: x=\(point.x), y=\(point.y)")
            // Example: Update UI or send points to a renderer
        }
    }
